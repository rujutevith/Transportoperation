...
const Cars = () => {
  ...
  const filteredCars = Array.isArray(cars) && cars.length > 0
    ? cars.filter(car => {
        ...
        const filterMatch = filterType === 'all' || (car.fuel_type || '') === filterType;
      ...
      {filteredCars.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCars.map((car) => (
            <div key={car.id} className="bg-gray-900 rounded-xl overflow-hidden hover:scale-105 transition duration-300">
              <img 
              ...
              <div className="p-6">
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between text-gray-300">
                    <span className="flex items-center gap-2"><Fuel className="w-4 h-4" /> Fuel</span>
                    <span>{car.fuel_type}</span>
                  </div>
                  <div className="flex justify-between text-gray-300">
                    <span className="flex items-center gap-2"><Settings className="w-4 h-4" /> Transmission</span>
